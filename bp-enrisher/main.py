...
def build_data():
    ...
    datas_table = datas_table.iloc[:, [0, 1, 7, 10, 11, 12, 13, 14, 15, 16]]  # Keep only the first 18 columns to avoid issues with unexpected extra columns
    datas_table.columns =  [
            "Business Partner",
            "Grp.",
            "Name 1",
    ...
    datas_table = datas_table[~personnes]
    ...
    join_table = join_table.iloc[:, :2]
    join_table.columns = ["Business Partner", "Addr. No."]
    ...
    if "country" in merged.columns:
        merged = merged[merged["country"].isin(["FR", "F", None])]
...
def merge_address(datas: pd.DataFrame, join_table: pd.DataFrame, adress_table: pd.DataFrame):
    """
    Assemble les informations d'adresse en alignant les tables SAP :
    fusion join_table + adress_table sur Addr. No., puis jointure sur BP.
    """
    datas = datas.copy()
    join_table = join_table.copy()
    adress_table = adress_table.copy()

    # Rename BP column in datas if needed
    if "Business Partner" in datas.columns and "BP" not in datas.columns:
        datas = datas.rename(columns={"Business Partner": "BP"})
    if "BP" not in datas.columns:
        warn("[merge] main BP column missing, skipping address merge")
        return datas

    # Rename join_table columns
    if "Business Partner" in join_table.columns:
        join_table = join_table.rename(columns={"Business Partner": "BP"})

    # Prepare ADRC with named columns via iloc (same as partner_processing.py)
    if not adress_table.empty and len(adress_table.columns) >= 30:
        adress_table = adress_table.iloc[:, [0, 4, 5, 11, 20, 26, 29]]
        adress_table.columns = ["Addr. No.", "postcode", "city", "country", "street5", "street", "street4"]
        adress_table = adress_table.drop_duplicates(subset=["Addr. No."])
    else:
        warn("[merge] address table empty or too few columns, skipping address fields")
        adress_table = pd.DataFrame(columns=["Addr. No."])

    # Merge join_table with adress_table to get full address details per BP
    address_lookup = pd.merge(left=join_table, right=adress_table, on="Addr. No.", how="left")

    # Final merge with main datas table
