...
PATH = BASE_DIR / "EXPORT_BUT000.xlsx"
...
def load_but000(
    ...
    if keep_columns is not None:
        ...
        if missing:
            raise ValueError(
                "EXPORT_BUT000.xlsx: missing columns: " + ", ".join(missing)
            )
    ...
    if add_standard_cols:
        if raw_df.shape[1] <= ADDR_INDEX:
            raise ValueError(
                f"EXPORT_BUT000.xlsx malformed: expected at least {ADDR_INDEX + 1} columns, got {raw_df.shape[1]}"
            )
        df["BP"] = _norm_customer(raw_df.iloc[:, BP_INDEX])
